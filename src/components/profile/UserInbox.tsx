...
export function UserInbox() {
  const t = useTranslations('UserInbox');
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
        ...
        await updateDoc(chatRef, {
            ...
            // unreadCount: increment(1) // Could add this for creator side notification
        });
    ...
    <div className="grid grid-cols-1 md:grid-cols-12 h-[600px] glass-card border-white/10 overflow-hidden rounded-[2rem]">
      ...
      <div className="md:col-span-4 border-r border-white/5 flex flex-col bg-white/[0.02]">
        <div className="p-4 border-b border-white/5">
            <h3 className="font-headline font-bold text-lg px-2">{t('creators')}</h3>
        </div>
                    ...
                    {chats.map(chat => (
                        ...
                        >
                            <Avatar className="h-10 w-10 border border-white/10">
                                <AvatarImage src={chat.creatorAvatar} />
                                <AvatarFallback>{chat.creatorName?.[0]}</AvatarFallback>
                            </Avatar>
                            <div className="flex-1 min-w-0">
                                <p className="font-bold text-sm truncate">{chat.creatorName}</p>
      ...
      <div className="md:col-span-8 flex flex-col bg-black/20">
        {selectedChat ? (
            <>
                ...
                <header className="p-4 border-b border-white/5 flex items-center gap-3">
                    <Avatar className="h-8 w-8 border border-white/10">
                        <AvatarImage src={selectedChat.creatorAvatar} />
                        <AvatarFallback>{selectedChat.creatorName?.[0]}</AvatarFallback>
                    </Avatar>
                    <span className="font-bold">{selectedChat.creatorName}</span>
                </header>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
                ...
                <h3 className="text-xl font-bold">{t('selectCreator')}</h3>
                <p className="text-sm text-muted-foreground mt-2 max-w-xs">{t('selectCreatorDesc')}</p>
            </div>
        )}
      </div>
    </div>
